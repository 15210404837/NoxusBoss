...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float4 color = tex2D(uImage0, frac(coords + orthogonalSeamDirection * sqrt(seamLengthFactor) * offsetsAreAllowed * -0.015));
    float2 warpOffset = tex2D(uImage1, coords * 0.8 + float2(uTime * 0.04, 0)).rg - 0.5;
    float4 scrollNoise = tex2D(uImage1, coords * 0.3 + float2(uTime * -0.1, 0) + warpOffset * warpIntensity);
