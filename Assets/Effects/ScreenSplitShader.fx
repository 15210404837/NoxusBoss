...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    // Calculate colors.
    float4 baseColor = tex2D(uImage0, coords + offset * offsetsAreAllowed) + brightnessBoost;
    float4 backgroundDimensionColor1 = tex2D(uImage1, coords * splitTextureZoomFactor + float2(uTime, 0) * -0.23) * splitBrightnessFactor;
    float4 backgroundDimensionColor2 = tex2D(uImage1, coords + backgroundDimensionColor1.rb * splitTextureZoomFactor * 0.12) * splitBrightnessFactor * 0.5;
