...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float4 result = tex2D(uImage1, clamp(mapCoords, float2(0.01, 0), float2(0.96, 1)));
    
    // Significantly increase the constrast of the result. This results in bright colors being exaggerated while dark colors shoot away towards a pitch black, similar to Xeroc's censor.
