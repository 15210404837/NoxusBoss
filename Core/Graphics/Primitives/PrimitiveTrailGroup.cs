    ...
    public class PrimitiveTrailGroup
    {
        ...
        public PrimitiveTrailGroup(PrimitiveGroupDrawContext drawContext, ManagedShader shader, int maxIndices, int maxVertices, Action shaderPreparations = null)
        {
            ...
            Main.QueueMainThreadAction(() =>
            {
                ...
                Vertices = new(gd, PrimitiveTrailCopy.VertexPosition2DColor.StaticVertexDeclaration, maxVertices, BufferUsage.WriteOnly);
            });
        }
        ...
        public void Draw(bool pixelated)
        {
            ...
            PrimitiveTrailCopy.VertexPosition2DColor[] vertices = new PrimitiveTrailCopy.VertexPosition2DColor[primitives.Sum(p => p.Vertices is null || p.CompletelyOffscreen ? 0 : p.Vertices.Length)];
            for (int i = 0; i < primitives.Count; i++)
            {
                ...
                for (int j = 0; j < p.Vertices.Length; j++)
                    vertices[vertexOffset + j] = p.Vertices[j];
