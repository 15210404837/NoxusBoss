...
{
    public class XerocKeyboardShader : ChromaShader
    {
        ...
        public XerocKeyboardShader()
        {
            ...
            if (Main.netMode != NetmodeID.Server)
            {
                ...
                Texture2D eyeTexture = ModContent.Request<Texture2D>("NoxusBoss/Core/Graphics/Shaders/Keyboard/XerocEye", AssetRequestMode.ImmediateLoad).Value;
        ...
        private void ProcessLowDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
        {
            for (int i = 0; i < fragment.Count; i++)
            {
                ...
                if (vibrancyInterpolant > 0f)
                {
                    ...
                    Vector3 psychedelicColor = Palette(colorInterpolant, XerocBoss.WingColorShift, new(0.5f, 0.5f, 0.2f), new(1f, 1f, 1f), new(0f, 0.333f, 0.667f)) * 0.8f;
                ...
                // Draw the eye.
                if (EyeBrightness > 0f && XerocBoss.Myself is not null && DarknessIntensity < 0.9f)
                {
                    float distanceFromXeroc = Main.screenPosition.X + Main.screenWidth * 0.5f - XerocBoss.Myself.Center.X;
                    float horizontalEyeOffset = Pow(Sin(time * 0.44f), 7f) * 4f + Clamp(distanceFromXeroc * 0.004f, -7f, 7f);
